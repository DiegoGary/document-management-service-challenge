...
public interface DocumentRepository extends JpaRepository<Document, Integer> {
  ...
  @Query(
      "SELECT DISTINCT d FROM Document d JOIN d.user u JOIN d.tags t WHERE "
  ...
  @Query(
      "SELECT DISTINCT d FROM Document d JOIN d.user u WHERE "
  ...
  @Query("SELECT DISTINCT d FROM Document d JOIN d.user u WHERE " + "u.name LIKE %:username%")
  Page<Document> findAllByUsername(String username, Pageable pageable);

  @Query("SELECT DISTINCT d FROM Document d JOIN d.user u WHERE " + "d.name LIKE %:name%")
  Page<Document> findAllByName(String filename, Pageable pageable);

  @Query(
      "SELECT DISTINCT d FROM Document d JOIN d.tags t WHERE "
          + "t.tag IN :tags "
          + "GROUP BY d.id HAVING COUNT(DISTINCT t.tag) = 1")
